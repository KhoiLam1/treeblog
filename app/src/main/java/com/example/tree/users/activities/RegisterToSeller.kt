...
import com.example.tree.databinding.ActivityRegisterToSellerBinding
...
class RegisterToSeller : AppCompatActivity() {
    ...
    private fun validateForm(): Boolean {
        ...
        if (binding.etShopName.text.toString().trim().isEmpty()) {
            binding.etShopName.error = "Writer name is required"
            isValid = false
        }
        ...
        if (validateForm()) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    runOnUiThread {
                        CustomToast.show(this@RegisterToSeller, "Writer created successfully", ToastType.SUCCESS)
